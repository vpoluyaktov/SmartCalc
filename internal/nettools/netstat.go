...
// HTTPNetstat performs an HTTP request and shows detailed timing breakdown
func HTTPNetstat(host string, port int) (string, error) {
	// Build URL
	scheme := "https"
	if port == 80 {
		scheme = "http"
	...
	// Format output
	output := fmt.Sprintf("\n> HTTP Netstat to %s:%d (%s)", host, port, scheme)
	if remoteAddr != "" {
